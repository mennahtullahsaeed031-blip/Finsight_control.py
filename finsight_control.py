...
def process_pl(summary, line_items):
...
with st.sidebar:
    ...
    <div style='text-align:center;padding:20px 0 10px'>
        ...
        <div style='color:#8BA3C7;font-size:11px;letter-spacing:0.12em;text-transform:uppercase'>
            CFO Intelligence Platform</div>
    </div>
    """, unsafe_allow_html=True)
    st.markdown("---")
    uploaded = st.file_uploader("📂 Upload Financial File", type=["xlsx","xls"], label_visibility="collapsed")
    use_demo = st.button("🎯 Try Demo Data (NileTech)", use_container_width=True)
    st.markdown("---")
    st.markdown("<div style='color:#8BA3C7;font-size:11px;font-weight:700'>PIPELINE</div>", unsafe_allow_html=True)
    for step in ["🗂️ Classify File","📊 Map P&L","📉 Variances","🎯 KPIs",
                 "🔍 Root Cause","🚨 Alerts","🔮 Forecast","💡 Recommendations","🏗️ Budget Builder"]:
...
# ── UPLOAD ────────────────────────────────────────────────────────────────────
    ...
    if st.session_state.get("file_key") != file_key:
        with st.spinner("🔍 Analyzing file..."):
            ...
            # Step 2: جرب P&L أولاً لأي ملف
            mapped  = map_file(tmp_path)
            pl_data = mapped.get("pl_data") or {}
            pl_sum  = pl_data.get("summary", {})
            pl_items = pl_data.get("line_items", [])

            if pl_sum.get("total_revenue", 0) > 0:
                # نجح → P&L
                an = {
                    "type":       "P&L",
                    "summary":    pl_sum,
...
if not data_loaded:
    ...
    <div style='display:flex;flex-direction:column;align-items:center;
        ...
        <div style='display:flex;gap:12px;flex-wrap:wrap;justify-content:center'>
            <span style='background:#111F38;border:1px solid #1E3A5F;color:#8BA3C7;border-radius:8px;padding:6px 14px;font-size:12px'>📊 P&L</span>
            <span style='background:#111F38;border:1px solid #1E3A5F;color:#8BA3C7;border-radius:8px;padding:6px 14px;font-size:12px'>🏦 Balance Sheet</span>
            <span style='background:#111F38;border:1px solid #1E3A5F;color:#8BA3C7;border-radius:8px;padding:6px 14px;font-size:12px'>🏗️ Fixed Assets</span>
            <span style='background:#111F38;border:1px solid #1E3A5F;color:#8BA3C7;border-radius:8px;padding:6px 14px;font-size:12px'>👥 Payroll</span>
            <span style='background:#111F38;border:1px solid #1E3A5F;color:#8BA3C7;border-radius:8px;padding:6px 14px;font-size:12px'>📋 Budget</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
    ...
    <div style='display:flex;align-items:center;gap:10px;margin-bottom:16px;
        ...
        <div>
            ...
            <div style='color:#8BA3C7;font-size:12px'>
                Confidence: {cl["confidence"]:.0f}% · Sheets: {", ".join(cl.get("sheet_names",[]))}
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
# P&L DASHBOARD
# ══════════════════════════════════════════════════════════════════════════════
if is_pl and summary and line_items:
    variances, var_sum, kpis, kpi_cards, root_causes, alerts, alert_sum, forecasts, recs, exec_sum = process_pl(summary, line_items)
    ...
    with tab1:
        ...
        with c1:
            st.markdown(f"<div style='color:#F0F6FF;font-size:22px;font-weight:800'>{company_name} · {period}</div>", unsafe_allow_html=True)
        with c2:
            st.markdown(f"""<div class='fin-card' style='text-align:center;padding:12px'>
                ...
                <div style='color:{health_color};font-size:26px;font-weight:800;font-family:DM Mono'>{health_score:.0f}/100</div>
                <div style='color:{health_color};font-size:11px'>{health_label}</div>
        ...
        with c4:
            ...
            st.markdown(f"""<div class='fin-card' style='text-align:center;padding:12px'>
                ...
                <div style='color:{fc_c};font-size:11px'>{forecasts["insights"]["projected_revenue_growth"]:+.1f}%</div>
            </div>""", unsafe_allow_html=True)

        st.markdown("<div class='section-title' style='margin-top:16px'>Key Performance Indicators</div>", unsafe_allow_html=True)
        cols = st.columns(3)
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=df_monthly["month"],y=df_monthly["actual"],name="Actual",
            line=dict(color="#06B6D4",width=2.5),fill="tozeroy",fillcolor="rgba(6,182,212,0.08)"))
        fig.add_trace(go.Scatter(x=df_monthly["month"],y=df_monthly["budget"],name="Budget",
            line=dict(color="#3B82F6",width=1.5,dash="dot")))
        fig.update_layout(plot_bgcolor="#111F38",paper_bgcolor="#111F38",
            font=dict(color="#8BA3C7",family="DM Sans"),legend=dict(bgcolor="#111F38"),
    ...
    with tab2:
        ...
        for label,val,is_total,color in pl_items:
            ...
            bl = f"border-left:3px solid {color}" if is_total else "border-left:3px solid transparent"
            st.markdown(f"""
            <div style='display:flex;align-items:center;gap:12px;background:{bg};
                ...
                <div style='flex:1;height:8px;background:#1E3A5F;border-radius:4px;overflow:hidden'>
                    <div style='width:{bar_pct}%;height:100%;background:{color};border-radius:4px;opacity:{"1" if is_total else "0.6"}'></div>
                </div>
                <div style='flex:0 0 110px;text-align:right;color:{color};
                            font-size:{"15px" if is_total else "13px"};
                            font-weight:{"800" if is_total else "600"};font-family:DM Mono'>{fmt_money(val)}</div>
            </div>""", unsafe_allow_html=True)

        st.markdown("<div class='section-title' style='margin-top:20px'>Budget vs Actual</div>", unsafe_allow_html=True)
        var_rows = []
        ...
        with c2:
            ...
            bar = go.Figure()
            bar.add_trace(go.Bar(x=df_monthly["month"],y=df_monthly["actual"],name="Revenue",marker_color="#06B6D4",opacity=0.85))
            bar.add_trace(go.Bar(x=df_monthly["month"],y=[v*s["gross_margin"] for v in df_monthly["actual"]],name="Gross Profit",marker_color="#10B981",opacity=0.85))
            bar.update_layout(plot_bgcolor="#111F38",paper_bgcolor="#111F38",font=dict(color="#8BA3C7"),
                barmode="group",legend=dict(bgcolor="#111F38"),margin=dict(l=0,r=0,t=10,b=0),height=220,
                xaxis=dict(gridcolor="#1E3A5F"),yaxis=dict(gridcolor="#1E3A5F",tickformat="$,.0f"))
    ...
    with tab3:
        ...
        with c1:
            ...
            for label,count,color in [("Critical",alert_sum["critical"],"#EF4444"),("High",alert_sum["high"],"#F97316"),("Total",alert_sum["total"],"#3B82F6")]:
                st.markdown(f"""<div style='background:{color}15;border:1px solid {color}40;border-radius:10px;padding:12px 20px;text-align:center;margin-bottom:10px'>
            ...
            for alert in alerts:
                ...
                st.markdown(f"""<div class='alert-{sev}'>
                    <div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:4px'>
                        <div style='color:{tc};font-weight:700;font-size:13px'>{alert.get("icon","⚠️")} {alert["title"]}</div>
                        <span class='badge badge-{sev}'>{alert["severity"]}</span>
                    </div>
                    <div style='color:#8BA3C7;font-size:12px;line-height:1.5'>{alert["message"]}</div>
                </div>""", unsafe_allow_html=True)

        st.markdown("<div class='section-title' style='margin-top:20px'>Root Cause Analysis</div>", unsafe_allow_html=True)
        for rc in root_causes:
            bg,border,text = {"Critical":("#3B0A0A","#EF4444","#FCA5A5"),"High":("#2D1B00","#F97316","#FCD34D"),"Medium":("#1A2200","#EAB308","#FDE68A")}.get(rc["severity"],("#111F38","#1E3A5F","#F0F6FF"))
            st.markdown(f"""<div style='background:{bg};border:1px solid {border};border-radius:10px;padding:16px;margin-bottom:12px'>
                <div style='display:flex;align-items:center;gap:10px;margin-bottom:8px'>
                    <span style='font-size:20px'>{rc["icon"]}</span>
                    <span style='color:{text};font-size:14px;font-weight:700'>{rc["title"]}</span>
                    <span style='margin-left:auto;background:{border};color:#fff;font-size:10px;font-weight:800;border-radius:4px;padding:2px 7px'>{rc["severity"]}</span>
                </div>
                <p style='color:#8BA3C7;font-size:13px;margin:0 0 8px;line-height:1.6'>{rc["root_cause"]}</p>
                <div style='background:#0F1E3C;border-radius:6px;padding:8px 12px;color:#F59E0B;font-size:12px;font-weight:600'>📌 Impact: {rc["impact"]}</div>
            </div>""", unsafe_allow_html=True)

    with tab4:
        fc_table = forecasts["table"]
        fc_ins   = forecasts["insights"]
        cols = st.columns(3)
        ...
        hist = monthly_data[-6:]
        fig_fc = go.Figure()
        fig_fc.add_trace(go.Scatter(x=[m["month"] for m in hist],y=[m["actual"] for m in hist],
            name="Historical",line=dict(color="#06B6D4",width=2.5),fill="tozeroy",fillcolor="rgba(6,182,212,0.08)"))
        fig_fc.add_trace(go.Scatter(x=[r["month"] for r in fc_table],y=[r["revenue"] for r in fc_table],
            name="Forecast",line=dict(color="#8B5CF6",width=2.5,dash="dot"),
        ...
        out_c = "#10B981" if fc_ins["outlook"]=="Positive" else "#F59E0B" if fc_ins["outlook"]=="Stable" else "#EF4444"
        c1,c2,c3 = st.columns(3)
        for col,label,val,color in [
            (c1,"Revenue Growth",f"{fc_ins['projected_revenue_growth']:+.1f}%","#10B981" if fc_ins['projected_revenue_growth']>0 else "#EF4444"),
            (c2,"Net Margin",f"{fc_ins['projected_net_margin']:.1f}%","#10B981"),
            (c3,"Outlook",fc_ins["outlook"],out_c),
        ]:
            with col:
    ...
    with tab5:
        for rec in recs:
            pc = {"Critical":"#EF4444","High":"#F97316","Medium":"#EAB308","Low":"#10B981"}.get(rec["priority"],"#8BA3C7")
            actions_html = "".join([f"<div style='display:flex;gap:8px;margin-bottom:6px;font-size:13px;color:#F0F6FF'><div style='background:#1E5CC8;color:#fff;border-radius:50%;width:20px;height:20px;font-size:10px;font-weight:800;display:flex;align-items:center;justify-content:center;flex-shrink:0'>{j+1}</div><div>{a}</div></div>" for j,a in enumerate(rec["actions"])])
            st.markdown(f"""<div class='rec-card' style='border-left:4px solid {pc}'>
                <div style='display:flex;align-items:center;gap:8px;margin-bottom:8px'>
                    <span style='background:{pc}20;color:{pc};font-size:10px;font-weight:800;border-radius:4px;padding:2px 7px'>{rec["priority"]}</span>
                ...
                <div style='color:#8BA3C7;font-size:12px;margin-bottom:12px'><strong style='color:#F59E0B'>Problem:</strong> {rec["problem"]}</div>
...
# NON-P&L DASHBOARD
# ══════════════════════════════════════════════════════════════════════════════
elif not is_pl and analysis and file_type not in ("UNKNOWN","ERROR"):
    an = analysis
    with tab1:
        st.markdown(f"<div style='color:#F0F6FF;font-size:22px;font-weight:800;margin-bottom:16px'>{company_name} · {period}</div>", unsafe_allow_html=True)
        st.markdown("<div class='section-title'>Key Metrics</div>", unsafe_allow_html=True)
        kpi_cards = an.get("kpi_cards",[])
        cols = st.columns(3)
        for i,card in enumerate(kpi_cards):
            color = {"good":"#10B981","warning":"#F59E0B","critical":"#EF4444"}.get(card.get("status","good"),"#8BA3C7")
            with cols[i%3]:
                st.markdown(f"""<div class='kpi-card' style='border-top:2px solid {color}'>
        ...
        for alert in an.get("alerts",[]):
            sev = alert["severity"].lower()
            bc = {"critical":"#EF4444","high":"#F97316","medium":"#EAB308"}.get(sev,"#8BA3C7")
            bg = {"critical":"#3B0A0A","high":"#2D1B00","medium":"#1A2200"}.get(sev,"#111F38")
            st.markdown(f"""<div style='background:{bg};border:1px solid {bc};border-radius:10px;padding:12px 14px;margin-bottom:8px'>
                <div style='display:flex;justify-content:space-between;margin-bottom:4px'>
                    <span style='color:{bc};font-weight:700;font-size:13px'>{alert.get("icon","⚠️")} {alert["title"]}</span>
                    <span style='background:{bc};color:#fff;font-size:10px;font-weight:800;border-radius:4px;padding:2px 7px'>{alert["severity"]}</span>
                </div>
                <div style='color:#8BA3C7;font-size:12px'>{alert["message"]}</div>
            </div>""", unsafe_allow_html=True)

        charts = an.get("charts",{})
        if charts.get("top_assets"):
            items = charts["top_assets"]
            fig = go.Figure()
            fig.add_trace(go.Bar(name="Budget",x=[i["name"][:20] for i in items],y=[i.get("budget",0) for i in items],marker_color="#3B82F6",opacity=0.8))
            fig.add_trace(go.Bar(name="Actual",x=[i["name"][:20] for i in items],y=[i.get("actual",0) for i in items],marker_color="#10B981",opacity=0.8))
            fig.update_layout(plot_bgcolor="#111F38",paper_bgcolor="#111F38",font=dict(color="#8BA3C7"),barmode="group",
                xaxis=dict(gridcolor="#1E3A5F",tickangle=-30),yaxis=dict(gridcolor="#1E3A5F",tickformat="$,.0f"),
                legend=dict(bgcolor="#111F38"),margin=dict(l=0,r=0,t=10,b=80),height=300)
            st.plotly_chart(fig,use_container_width=True)

        pie_data = charts.get("by_category") or charts.get("by_dept") or charts.get("structure",[])
        if pie_data:
            pie = go.Figure(go.Pie(labels=[d["name"] for d in pie_data],values=[d["value"] for d in pie_data],hole=0.55,textfont=dict(color="#F0F6FF")))
            pie.update_layout(plot_bgcolor="#111F38",paper_bgcolor="#111F38",font=dict(color="#8BA3C7"),legend=dict(bgcolor="#111F38"),margin=dict(l=0,r=0,t=10,b=0),height=280)
            st.plotly_chart(pie,use_container_width=True)

    for t in [tab2,tab3,tab4,tab5]:
        with t:
            st.info("Available for P&L files only. Upload an Income Statement to unlock.")
...
# ══════════════════════════════════════════════════════════════════════════════
with tab6:
    from budget_builder import build_master_budget, run_scenarios, explain_budget, DEFAULT_ASSUMPTIONS

    st.markdown("""
    <div style='background:linear-gradient(135deg,#0A1628,#0F2040);border:1px solid #8B5CF640;
                border-radius:12px;padding:20px;margin-bottom:20px'>
        <div style='color:#8B5CF6;font-size:13px;font-weight:700;text-transform:uppercase;
                    letter-spacing:0.08em;margin-bottom:8px'>🏗️ FP&A Budget Builder</div>
        <div style='color:#8BA3C7;font-size:13px;line-height:1.7'>
            Enter your assumptions and FinSight builds a complete Master Budget automatically.
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.button("🚀 Build Master Budget", use_container_width=True):
        with st.spinner("Building complete budget..."):
            assumptions = {
                **DEFAULT_ASSUMPTIONS,
                "base_revenue":base_rev,"revenue_growth":rev_growth,
                "price_per_unit":price_unit,"cogs_pct":cogs_pct,
                "inflation_rate":inflation,"material_per_unit":mat_cost,
                "labor_hours_per_unit":labor_hrs,"labor_rate_per_hour":labor_rate,
                "base_headcount":headcount,"avg_salary_monthly":avg_salary,
                "loan_balance":loan_bal,"interest_rate":int_rate,
                "tax_rate":tax_rate,"capex_annual":capex,
            }
    ...
    if "master_budget" in st.session_state:
        ...
        st.markdown("<div class='section-title'>Budgeted KPIs</div>", unsafe_allow_html=True)
        cols = st.columns(6)
        for col,label,val,color in [
            (cols[0],"Revenue",    fmt_money(is_data["annual_revenue"]), "#06B6D4"),
            (cols[1],"Gross Profit",fmt_money(is_data["annual_gp"]),    "#10B981"),
            (cols[2],"Net Income", fmt_money(is_data["annual_net"]),     "#10B981" if is_data["annual_net"]>0 else "#EF4444"),
            (cols[3],"Gross Margin",f"{is_data['avg_gm']:.1f}%",        "#10B981"),
            (cols[4],"Net Margin", f"{is_data['avg_nm']:.1f}%",         "#10B981" if is_data["avg_nm"]>10 else "#F59E0B"),
            (cols[5],"Closing Cash",fmt_money(cf_data["closing"]),      "#10B981" if cf_data["closing"]>0 else "#EF4444"),
        ]:
            with col:
                st.markdown(f"""<div class='fin-card' style='text-align:center;padding:12px'>
                    <div style='color:#8BA3C7;font-size:10px;font-weight:700;text-transform:uppercase'>{label}</div>
                    <div style='color:{color};font-size:18px;font-weight:800;font-family:DM Mono;margin-top:6px'>{val}</div>
                </div>""", unsafe_allow_html=True)

        st.markdown("<div class='section-title' style='margin-top:20px'>AI Budget Insights</div>", unsafe_allow_html=True)
        for ins in insights:
            st.markdown(f"<div style='background:#111F38;border:1px solid #1E3A5F;border-left:3px solid #8B5CF6;border-radius:8px;padding:12px 16px;margin-bottom:8px;color:#8BA3C7;font-size:13px;line-height:1.6'>{ins}</div>", unsafe_allow_html=True)

        st.markdown("<div class='section-title' style='margin-top:20px'>Monthly Revenue Budget</div>", unsafe_allow_html=True)
        rev_data = master["revenue"]["monthly"]
        fig_r = go.Figure(go.Bar(x=[m["month"] for m in rev_data],y=[m["revenue"] for m in rev_data],marker_color="#06B6D4",opacity=0.85))
        fig_r.update_layout(plot_bgcolor="#111F38",paper_bgcolor="#111F38",font=dict(color="#8BA3C7"),
            xaxis=dict(gridcolor="#1E3A5F"),yaxis=dict(gridcolor="#1E3A5F",tickformat="$,.0f"),
            margin=dict(l=0,r=0,t=10,b=0),height=220)
        st.plotly_chart(fig_r,use_container_width=True)

        st.markdown("<div class='section-title'>Budgeted Income Statement</div>", unsafe_allow_html=True)
        st.dataframe(pd.DataFrame([{
            "Month":m["month"],"Revenue":fmt_money(m["revenue"]),"COGS":fmt_money(m["cogs"]),
            "Gross Profit":fmt_money(m["gross_profit"]),"GM%":f"{m['gross_margin']:.1f}%",
            "SG&A":fmt_money(m["sga"]),"EBIT":fmt_money(m["ebit"]),
            "Net Income":fmt_money(m["net_income"]),"NM%":f"{m['net_margin']:.1f}%",
        } for m in is_data["monthly"]]),use_container_width=True,hide_index=True)

        st.markdown("<div class='section-title' style='margin-top:20px'>What-If Scenarios</div>", unsafe_allow_html=True)
        sc_cols = st.columns(4)
        sc_colors = {"Base Case":"#3B82F6","Optimistic":"#10B981","Pessimistic":"#EF4444","High Inflation":"#F59E0B"}
        for col,(name,vals) in zip(sc_cols,scenarios.items()):
            color = sc_colors.get(name,"#8BA3C7")
            with col:
                st.markdown(f"""<div class='fin-card' style='border-top:3px solid {color};text-align:center'>
                    <div style='color:{color};font-size:12px;font-weight:800;margin-bottom:10px'>{name}</div>
                    <div style='color:#06B6D4;font-size:13px;margin-bottom:4px'>Rev: {fmt_money(vals["revenue"])}</div>
                    <div style='color:#10B981;font-size:13px;margin-bottom:4px'>Net: {fmt_money(vals["net_income"])}</div>
                    <div style='color:#F59E0B;font-size:12px'>GM: {vals["gross_margin"]:.1f}% | NM: {vals["net_margin"]:.1f}%</div>
                </div>""", unsafe_allow_html=True)

        st.markdown("<div class='section-title' style='margin-top:20px'>Cash Flow Projection</div>", unsafe_allow_html=True)
        cf_m = cf_data["monthly"]
        fig_cf = go.Figure()
        fig_cf.add_trace(go.Scatter(x=[m["month"] for m in cf_m],y=[m["closing_cash"] for m in cf_m],
            name="Closing Cash",line=dict(color="#10B981",width=2.5),fill="tozeroy",fillcolor="rgba(16,185,129,0.08)"))
        fig_cf.add_trace(go.Bar(x=[m["month"] for m in cf_m],y=[m["net_cash"] for m in cf_m],
            name="Net Cash",marker_color=["#10B981" if m["net_cash"]>=0 else "#EF4444" for m in cf_m],opacity=0.7))
        fig_cf.update_layout(plot_bgcolor="#111F38",paper_bgcolor="#111F38",font=dict(color="#8BA3C7"),
            legend=dict(bgcolor="#111F38"),xaxis=dict(gridcolor="#1E3A5F"),
            yaxis=dict(gridcolor="#1E3A5F",tickformat="$,.0f"),margin=dict(l=0,r=0,t=10,b=0),height=250)
