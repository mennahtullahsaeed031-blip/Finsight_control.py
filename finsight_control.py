...
with st.sidebar:
    ...
    for step in ["🗂️ Classify File","📊 Map P&L","📉 Variances",
                 "🎯 KPIs","🔍 Root Cause","🚨 Alerts","🔮 Forecast","💡 Recommendations"]:
        st.markdown(
...
# ── CASE 2: Uploaded File ─────────────────────────────────────────────────────
    ...
    if st.session_state.get("file_key") != file_key:
        ...
        with st.spinner("🔍 Classifying and analyzing file..."):
            ...
            # Step 3: Save everything to session
...
# ══════════════════════════════════════════════════════════════════════════════
# DASHBOARD — بيشتغل بس لما data_loaded = True
# ══════════════════════════════════════════════════════════════════════════════

# ── File Type Badge (لو ملف مرفوع مش demo) ───────────────────────────────────
if classifier:
    cl = classifier
    st.markdown(f"""
...
if file_type == "P&L" and summary and line_items:
    ...
    with col4:
        ...
        st.markdown(f"""<div class='fin-card' style='text-align:center;padding:12px'>
            ...
            <div style='color:{fc_c};font-size:11px'>
    ...
    with tab1:
        ...
        fig.add_trace(go.Scatter(
            x=df_monthly["month"], y=df_monthly["actual"], name="Actual",
            line=dict(color="#06B6D4",width=2.5),
            fill="tozeroy", fillcolor="rgba(6,182,212,0.08)"))
        fig.add_trace(go.Scatter(
            x=df_monthly["month"], y=df_monthly["budget"], name="Budget",
...
# NON-P&L DASHBOARDS
# ══════════════════════════════════════════════════════════════════════════════
elif file_type in ("FIXED_ASSETS","BALANCE_SHEET","PAYROLL","BUDGET") and analysis:

    an = analysis
    st.markdown(f"<div style='color:#F0F6FF;font-size:22px;font-weight:800;margin-bottom:16px'>"
                f"{company_name} · {period}</div>", unsafe_allow_html=True)
    ...
    if file_type == "FIXED_ASSETS" and charts.get("top_assets"):
        st.markdown("<div class='section-title'>Top Assets — Budget vs Actual</div>",
                    unsafe_allow_html=True)
        items = charts["top_assets"]
        fig = go.Figure()
        names = [i["name"][:25] for i in items]
        fig.add_trace(go.Bar(name="Budget",x=names,
            y=[i["budget"] for i in items],marker_color="#3B82F6",opacity=0.8))
        fig.add_trace(go.Bar(name="Actual",x=names,
            y=[i["actual"] for i in items],marker_color="#10B981",opacity=0.8))
        fig.update_layout(
            plot_bgcolor="#111F38",paper_bgcolor="#111F38",
            font=dict(color="#8BA3C7"),barmode="group",
    ...
    if charts.get("by_category") or charts.get("by_dept") or charts.get("structure"):
